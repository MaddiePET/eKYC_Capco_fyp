...
export default function UserDropdown() {
  ...
  const [currentAccount, setCurrentAccount] = useState("Musharof Chowdhury");
  ...
  const accounts = [
    ...
    },
    {
      id: 3,
      name: "Boleh Sdn Bhd",
      email: "boleh.sdnbhd@gmail.com",
      avatar: "/images/user/user-03.jpg",
    },
  ];

  // Load account from localStorage only after hydration
  ...
  const activeAccount = accounts.find((acc) => acc.name === currentAccount);
  ...
  function switchAccount(accountName: string) {
    if (isProfilePage) {
      closeDropdown();
      return;
    }
      ...
      >
        <span className="mr-3 overflow-hidden rounded-full h-11 w-11">
          <Image
            width={44}
            height={44}
            src={activeAccount?.avatar || "/images/user/owner.jpg"}
            alt="User"
          />
        </span>

        <span className="block mr-1 font-medium text-theme-sm">{activeAccount?.name.split(" ")[0]}</span>
            ...
            {accounts.map((account) => (
              <li key={account.id}>
                <button
                  ...
                  className={`w-full flex items-center gap-2 px-3 py-2 rounded-lg text-theme-sm transition-colors ${
                    isProfilePage
                ...
                >
                  ...
                  <div className="text-left">
                    ...
                    <p className="text-[11px] opacity-75">{account.email}</p>
                  </div>
                  {currentAccount === account.name && (
                    <svg
                      className="w-4 h-4 ml-auto shrink-0 fill-current"
                      viewBox="0 0 24 24"
                      fill="none"
                      xmlns="http://www.w3.org/2000/svg"
                    >
