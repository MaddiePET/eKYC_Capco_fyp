...
export default function PersonalNonMalaysianAccountCreation() {
  ...
  const handleFinalSubmit = async () => {
    try {
      ...
      // 1. Core local storage key synchronization
      ...
      // Pull correct personal info from nonMsianInfo
      const personalInfo = JSON.parse(localStorage.getItem("nonMsianInfo") || "{}");
      
      // Pull single synchronized nonMsianAddress object wrapper
      ...
      // 2. Map Address properties exactly to the structural layout of address/page.tsx
      const homeAddress = {
        add_type: "Home",
      ...
      // 3. Confirm validation is satisfied
      if (!homeAddress.add_1 || !homeAddress.country) {
        throw new Error("Home address field Line 1 or Country parameter is missing. Please go back and re-fill the address page.");
      }

      // 4. Construct production request payload matching your backend route
      const payload = {
        journeyId,
        customer: {
      ...
      console.log("SENDING NON-MALAYSIAN APP payload verification:", JSON.stringify(payload));
      ...
      if (!response.ok) {
        throw new Error(result.error || "Failed to finalize account creation sequence.");
      }

      console.log("Non-Malaysian registration successful:", result);

      // Prune persistent caches cleanly on completion
      ...
      <div className="relative w-full max-w-md z-10">
              ...
              <div>
                ...
                <div className="mt-3 flex flex-wrap gap-2">
                  {phraseOptions.map((phrase, idx) => (
                    <button key={idx} type="button" onClick={() => setSecurityPhrase(phrase)} className="px-3 py-1.5 text-[11px] font-medium rounded-md border-2 text-gray-600 hover:border-[#3D405B]">{phrase}</button>
                  ))}
                </div>
              </div>
        ...
        {step === "pending" && (
          <div className="text-center animate-in fade-in duration-700">
            ...
            <button type="button" onClick={() => router.push("/")} className="w-full px-4 py-3 text-sm font-bold text-white transition rounded-lg bg-[#3D405B]">Finish</button>
          </div>
        )}
      </div>
    </div>
