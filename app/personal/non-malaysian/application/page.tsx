              ...
              {documents.map((doc, index) => (
                <div key={doc.id} className="p-6 bg-white border-2 border-gray-100 rounded-2xl dark:bg-gray-900/90 dark:border-gray-800 shadow-sm relative">
                  ...
                  <div className="mb-6">
                    ...
                    <input type="text" placeholder="e.g. Working Permit, Visa, Letter of Confirmation" className="w-full px-4 py-2.5 text-sm font-medium transition-all border-2 rounded-xl outline-none bg-white border-gray-200 text-gray-800 focus:border-[#F0CA8E] focus:ring-4 focus:ring-[#F0CA8E]/20 dark:bg-gray-900/90 dark:border-[#5c6185] dark:text-white dark:focus:border-[#F0CA8E] dark:focus:ring-[#3D405B]/40" value={doc.name} onChange={(e) => updateDoc(doc.id, { name: e.target.value })} />
                  </div>
                  <div onClick={() => document.getElementById(`file-${doc.id}`)?.click()} onDragOver={(e) => { e.preventDefault(); setDraggingId(doc.id); }} onDragLeave={() => setDraggingId(null)} onDrop={(e) => { e.preventDefault(); setDraggingId(null); handleFile(doc.id, e.dataTransfer.files?.[0]); }} className={`relative group cursor-pointer overflow-hidden rounded-xl border-2 transition-all duration-300 flex items-center px-4 py-3 ${draggingId === doc.id ? 'border-[#F0CA8E] bg-white shadow-lg ring-4 ring-[#F0CA8E]/20 dark:bg-gray-900/90 dark:border-[#F0CA8E]' : doc.preview ? 'border-[#F0CA8E] bg-white dark:bg-gray-900/90 dark:border-[#F0CA8E]' : 'border-2 border-dashed border-gray-200 bg-white hover:border-[#F0CA8E]/50 dark:border-gray-800 dark:bg-gray-900'}`}>
        ...
        {step === 3 && (
          <div className="animate-in fade-in slide-in-from-right-4 duration-500">
            ...
            <div className="pt-10 flex flex-col items-center">
              ...
              <button onClick={() => router.push("/personal/non-malaysian/account_creation")} disabled={!preferredBranch} className="inline-flex items-center justify-center w-full px-4 py-3 text-sm font-bold transition rounded-lg bg-[#3D405B] text-white hover:bg-[#2c2f42] dark:bg-[#3D405B] dark:hover:bg-[#4a4e6d] disabled:bg-gray-200 disabled:text-gray-400 disabled:cursor-not-allowed dark:disabled:bg-gray-800 dark:disabled:text-gray-600 shadow-theme-xs">
                Continue
              </button>
              <div className="mt-5 text-center">
                <p className="text-sm">
