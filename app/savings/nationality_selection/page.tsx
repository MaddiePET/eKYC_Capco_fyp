// app/savings/nationality_selection/page.tsx
"use client";
...
export default function SavingsNationalitySelection() {
  ...
  const searchParams = useSearchParams(); // Hook capture
  const [selectedNationality, setSelectedNationality] = useState<string>("");

  // Check if user came via the signup selection track
  const isFromSignup = searchParams.get("from") === "signup";

  const handleNext = () => {
    // Retain parameter context into next steps
  ...
  const handleBack = () => {
    if (isFromSignup) {
      ...
      router.push("/savings/user_verification"); // Fallback original design destination
    }
  };

  return (
    <div className="relative flex flex-col items-center justify-center min-h-screen px-4 py-20 bg-[#F9FAFB] dark:bg-gray-950 overflow-hidden">
      {/* Wave Background SVG Nodes remain identical */}
      ...
      <div className="relative w-full max-w-md z-10">
        <div className="mb-10 text-center">
          <h1 className="mb-3 font-bold text-gray-800 text-title-sm dark:text-white sm:text-title-md">Choose Your Nationality</h1> 
          <p className="text-sm text-gray-500 dark:text-gray-400">Please select your nationality to proceed with the registration.</p>
        </div>
