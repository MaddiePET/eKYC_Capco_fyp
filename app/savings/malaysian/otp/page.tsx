...
export default function SavingsMalaysianOTP() {
  ...
  const handleBack = () => {
    ...
    if (step === "otp") {
      ...
      router.push(`/savings/malaysian/face_verification?journeyId=${encodeURIComponent(journeyId)}`);
    }
  };
  ...
  const fetchAndRouteToInfo = async () => {
    try {
      ...
      router.push(`/savings/malaysian/info?id_type=ic&id_num=${encodeURIComponent(icNo)}&journeyId=${encodeURIComponent(journeyId)}`);
    } catch (error) {
