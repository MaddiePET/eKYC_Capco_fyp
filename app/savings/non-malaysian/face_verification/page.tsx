...
export default function SavingsNonMalaysianFaceQRCode() {
  ...
  useEffect(() => {
    ...
    const targetUrl = `${origin}/savings/non-malaysian/face_verification/mobile?journeyId=${jId}&theme=${activeTheme}`;
  ...
  const handleNext = () => {
    if (isVerified) {
      router.push(
        `/savings/non-malaysian/otp?journeyId=${encodeURIComponent(journeyId || "")}`
      );
    }
  };

  const handleBack = () => {
    router.push(`/savings/non-malaysian/passport?journeyId=${encodeURIComponent(journeyId || "")}`);
  };
