...
export default function CurrentMalaysianMyKadQRCode() {
  ...
  useEffect(() => {
    ...
    const targetUrl = `${origin}/current/malaysian/mykad/mobile?journeyId=${jId}&theme=${activeTheme}`;
  ...
  const handleNext = () => {
    if (isVerified) {
      router.push(
        `/current/malaysian/face_verification?journeyId=${encodeURIComponent(journeyId)}`
      );
    }
  };
