...
export default function CurrentMalaysianOTP() {
  ...
  const handleBack = () => {
    ...
    if (step === "otp") {
      ...
      router.push(`/current/malaysian/face_verification?journeyId=${encodeURIComponent(journeyId)}`);
    }
  };
  ...
  const fetchAndRouteToInfo = async () => {
    try {
      ...
      router.push(`/current/malaysian/info?id_type=ic&id_num=${encodeURIComponent(icNo)}&journeyId=${encodeURIComponent(journeyId)}`);
    } catch (error) {
