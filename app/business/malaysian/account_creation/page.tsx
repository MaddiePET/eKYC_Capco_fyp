          ...
          <div className="animate-in fade-in duration-500">
            ...
            <div className="space-y-6">
              <div className="flex flex-col items-center">
                <div onClick={() => fileInputRef.current?.click()} className={`relative group w-28 h-28 rounded-full border-2 border-dashed transition-all cursor-pointer flex items-center justify-center overflow-hidden bg-white dark:bg-gray-900 ${profilePreview ? "border-[#F0CA8E]" : "border-gray-300 dark:border-gray-700 hover:border-[#F0CA8E] dark:hover:border-[#F0CA8E]"}`}>
                  {profilePreview ? (
                    <>
                ...
                <div className="mt-4 flex gap-3">
                  {avatarOptions.map((url, idx) => (
                    <button key={idx} type="button" onClick={() => setProfilePreview(url)} className={`w-10 h-10 rounded-full border-2 transition-all overflow-hidden ${profilePreview === url ? 'border-[#3D405B] scale-110' : 'border-transparent hover:border-gray-300'}`}>
              ...
              <div>
                ...
                <input className="w-full px-4 py-2.5 text-sm transition-all bg-white border-2 rounded-xl outline-none border-gray-200 focus:border-[#F0CA8E] focus:ring-4 focus:ring-[#F0CA8E]/20 dark:bg-gray-900/90 dark:border-[#5c6185] dark:text-white" placeholder="Enter your username" value={username} onChange={(e) => setUsername(e.target.value.replace(/[^a-zA-Z0-9]/g, ""))} />
              </div>

              <button type="button" onClick={handleNext} disabled={!username || username.length < 5 || !profilePreview} className="inline-flex items-center justify-center w-full px-4 py-3 text-sm font-bold text-white transition rounded-lg bg-[#3D405B] shadow-theme-xs hover:bg-[#2c2f42] dark:bg-[#3D405B] disabled:bg-gray-200 disabled:text-gray-400 disabled:cursor-not-allowed">Continue</button>
            </div>
          </div>
        )}
          ...
          <div className="animate-in fade-in duration-500">
            ...
            <div className="space-y-5">
              <div>
                ...
                <input className="w-full px-4 py-2.5 text-sm transition-all bg-white border-2 rounded-xl outline-none border-gray-200 focus:border-[#F0CA8E]" placeholder="Enter your security phrase" value={securityPhrase} onChange={(e) => setSecurityPhrase(e.target.value.replace(/[^a-zA-Z!,.\s]/g, ""))} />
                <div className="mt-3 flex flex-wrap gap-2">
                  {phraseOptions.map((phrase, idx) => (
                    <button key={idx} type="button" onClick={() => setSecurityPhrase(phrase)} className="px-3 py-1.5 text-[11px] font-medium rounded-md border-2 border-gray-200 dark:border-gray-800 text-gray-600 dark:text-gray-400 hover:border-[#3D405B] hover:text-[#3D405B] transition-colors">{phrase}</button>
                  ))}
                </div>
              </div>
              ...
              <div>
                ...
                <div className="relative">
                  <input type={showPassword ? "text" : "password"} className="w-full px-4 py-2.5 text-sm transition-all bg-white border-2 rounded-xl outline-none border-gray-200 focus:border-[#F0CA8E]" placeholder="Enter your password" value={password} onChange={(e) => setPassword(e.target.value)} />
                ...
                {password.length > 0 && !isPasswordValid && (
                  <div className="mt-2 text-[10px] space-y-1 animate-in slide-in-from-top-1 fade-in duration-300">
                    ...
                    <p className={/[0-9]/.test(password) ? "text-green-500" : "text-gray-400"}>{/[0-9]/.test(password) ? "✓" : "○"} At least one number</p>
                    <p className={/[A-Z]/.test(password) ? "text-green-500" : "text-gray-400"}>{/[A-Z]/.test(password) ? "✓" : "○"} At least one capital letter</p>
                    <p className={/[a-z]/.test(password) ? "text-green-500" : "text-gray-400"}>{/[a-z]/.test(password) ? "✓" : "○"} At least one lowercase letter</p>
                    <p className={/[^A-Za-z0-9]/.test(password) ? "text-green-500" : "text-gray-400"}>{/[^A-Za-z0-9]/.test(password) ? "✓" : "○"} At least one special character</p>
                  </div>
                )}
                
                <div className="h-1 w-full bg-gray-200 dark:bg-gray-800 rounded-full mt-3 overflow-hidden">
                  <div className={`h-full transition-all duration-500 ${password.length === 0 ? 'w-0' : score <= 1 ? 'w-1/4 bg-red-500' : score === 2 ? 'w-2/4 bg-yellow-500' : score === 3 ? 'w-3/4 bg-blue-400' : 'w-full bg-green-500'}`} />
                </div>
                <p className={`text-[10px] mt-1 italic font-bold uppercase transition-colors ${score <= 1 ? 'text-red-500' : score === 2 ? 'text-yellow-600' : score === 3 ? 'text-blue-400' : 'text-green-500'}`}>{getPasswordStrength()}</p>
              </div>
              ...
              <div>
                ...
                <input type="password" className="w-full px-4 py-2.5 text-sm transition-all bg-white border-2 rounded-xl outline-none border-gray-200" placeholder="Confirm your password" value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} />
              </div>

              {submitError && <div className="p-3 text-sm border bg-red-50 text-red-600 rounded-lg text-center">{submitError}</div>}

              <button type="button" onClick={handleSubmit} disabled={!password || !securityPhrase || password !== confirmPassword || !isPasswordValid || isSubmitting} className="inline-flex items-center justify-center w-full px-4 py-3 text-sm font-bold text-white transition rounded-lg bg-[#3D405B] shadow-theme-xs hover:bg-[#2c2f42] disabled:bg-gray-200 disabled:text-gray-400 disabled:cursor-not-allowed">
                {isSubmitting ? "Processing Setup..." : "Create Account"}
              </button>
            </div>
          </div>
        )}
        ...
        {step === "pending" && (
          <div className="text-center animate-in fade-in duration-700">
            ...
            <div className="mb-10 p-4 rounded-xl border bg-blue-50 border-blue-200 dark:bg-blue-900/30 dark:border-blue-500/50">
              <p className="text-xs text-blue-900 dark:text-blue-100 leading-relaxed">Our team typically reviews applications within <span className="font-bold">24 hours</span>. Any additional documents needed will be informed via email.</p>
            </div>

            <button type="button" onClick={() => router.push("/")} className="inline-flex items-center justify-center w-full px-4 py-3 text-sm font-bold text-white transition rounded-lg bg-[#3D405B] shadow-theme-xs hover:bg-[#2c2f42] dark:bg-[#3D405B]">Finish</button>
          </div>
        )}
        ...
        {step !== "pending" && (
           <div className="mt-5 text-center">
             <p className="text-sm font-normal">
               ...
               <Link href="/support" className="font-semibold text-blue-700 hover:text-blue-800 dark:text-blue-400 transition-colors">Contact Support</Link>
             </p>
           </div>
        )}
      </div>
