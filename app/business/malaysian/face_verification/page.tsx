...
export default function BusinessMalaysianFaceQRCode() {
  ...
  useEffect(() => {
    // Get Journey ID from URL or Local Storage
    ...
    // URL pointing to the new Business Mobile Face Capture route
    const targetUrl = `${origin}/business/malaysian/face_verification/mobile?journeyId=${jId}`;
    ...
    // Poll for the status update triggered by the mobile phone
  ...
  const handleBack = () => {
    router.push(`/business/malaysian/mykad?journeyId=${encodeURIComponent(journeyId || "")}`);
  };
    ...
    <div className="relative flex flex-col items-center justify-center min-h-screen px-4 py-20 bg-[#F9FAFB] dark:bg-gray-950 overflow-hidden">
      {isFailed && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-gray-900/60 backdrop-blur-sm px-4">
          <div className="bg-white dark:bg-gray-900 rounded-3xl p-8 max-w-sm w-full text-center shadow-2xl border border-gray-200 dark:border-gray-800 animate-in fade-in zoom-in duration-300">
            ...
            <h2 className="text-xl font-bold text-gray-900 dark:text-white mb-2">Too Many Attempts</h2>
      ...
      <main className="relative w-full max-w-2xl z-10">
        <div className="mb-10 text-center">
          <h1 className="mb-3 font-bold text-gray-800 text-title-sm dark:text-white sm:text-title-md">Scan Your Face</h1>
              ...
              {mobileUrl ? (
                <div className="relative">
                  <QRCodeSVG
                    ...
                    size={220}
                    level="H"
                    className={`rounded-xl transition-all duration-500 ${isVerified || isFailed ? "opacity-30 blur-sm" : "opacity-100"}`}
                  />
                  {isVerified && (
