...
export default function AdminLayoutContent({ children }: { children: React.ReactNode }) {
  ...
  const activeAccount = useMemo(() => {
    ...
    if (loggedInUser && currentAccountName === loggedInUser.name) {
      return {
        ...
        isMalaysian: false, // Prevents unauthorized flashes of business creation while loading
        phone: ""
      };
    }
  ...
  // Dynamically filter Navigation Items (Hide Add Account for Personal Non-Malaysian)
  ...
  return navItems.filter((item) => {
    if (item.name === "Add Account") {
      // If Personal AND Not Malaysian, hide the "Add Account" menu
  ...
  const handleOpenAccountModal = () => {
    if (activeAccount?.type === "Personal") {
      if (activeAccount.isMalaysian) {
        // Personal Malaysian -> ONLY allow Business Account
        setCanCreatePersonal(false);
        setCanCreateBusiness(true);
      } else {
        // Personal Non-Malaysian (Catch-all, should be hidden from UI anyway)
        setCanCreatePersonal(false);
  ...
  const handleConfirmCreation = () => {
    if (selectedType === "Business") {
      ...
      if (activeAccount && !activeAccount.isMalaysian) {
            ...
            <nav className="mb-6">
              <div className="flex flex-col gap-4">
                <div>
                  ...
                  {/* Render using dynamically filtered items */}
